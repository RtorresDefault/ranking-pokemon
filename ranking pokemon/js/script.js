        ...
        const players = [
            {position: 1, player: "João Silva", points: 120, wins: 12, fidelidade: "5/8"},
            {position: 2, player: "Maria Oliveira", points: 115, wins: 11, fidelidade: "4/8"},
            {position: 3, player: "Pedro Santos", points: 110, wins: 11, fidelidade: "6/8"},
            {position: 4, player: "Ana Costa", points: 105, wins: 10, fidelidade: "3/8"},
            {position: 5, player: "Lucas Ferreira", points: 100, wins: 10, fidelidade: "7/8"},
        ...
        function getSortValue(row, column) {
            switch(column) {
                case 0: return row.position;
                case 1: return row.player.toLowerCase();
                case 2: return row.points;
                case 3: return row.wins;

                // Fidelidade ordena pelo primeiro número X do X/X
                case 4: return parseInt(row.fidelidade.split("/")[0]) || 0;
        ...
        function setupEventListeners() {
            ...
            document.getElementById('sortSelect').addEventListener('change', (e) => {
                const colMap = {position: 0, player: 1, points: 2, wins: 3, fidelidade: 4};
        ...
        function updateStats() {
            ...
            const avgPoints = currentData.reduce((sum, p) => sum + p.points, 0) / currentData.length || 0;
            document.getElementById('avgPoints').textContent = avgPoints.toFixed(1);

            // Maior fidelidade (pelo X)
            let top = currentData.reduce((best, p) => {
